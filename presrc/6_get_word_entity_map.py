...
def get_word_entity_map(args):
    map_file = os.path.join(args.dp, args.dn, args.ct + 'word_entity_map.txt')
    if not check_exist(map_file):
        # get vocab list
        vocab_f = os.path.join(args.dp, args.dn, args.ct + 'vocab.txt')
        with open(vocab_f, 'r') as f:
            vocab_l = f.read().splitlines()

            # get entity2id
        ent_file = os.path.join(args.dp, args.dn, args.ct + 'entity2id.txt')
...
def get_word_rel_map(args):
    map_file = os.path.join(args.dp, args.dn, args.ct + 'word_relation_map.txt')
    if not check_exist(map_file):
        # get vocab list
        vocab_f = os.path.join(args.dp, args.dn, args.ct + 'vocab.txt')
        with open(vocab_f, 'r') as f:
            ...
            # get entity2id
        ent_file = os.path.join(args.dp, args.dn, args.ct + 'relation2id.txt')
        ent_df = pd.read_csv(ent_file, sep='\t', names=['name', 'id'])
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(args.dp, args.dn, args.ct + 'vocab.txt')):
        print('Check if the vocab.txt exists')
        exit()

    if not os.path.exists(os.path.join(args.dp, args.dn, args.ct + 'entity2id.txt')):
