...
def get_token_vocab(args):
    text_f = os.path.join('../data', args.dn, args.ct + 'text_token.txt')
    ...
    vocab_f = os.path.join('../data', args.dn, args.ct + 'vocab.txt')
    ...
    for window in windows:
        for i in range(1, len(window)):
            for j in range(0, i):
                ...
                if word_id_map.get(word_i) is not None and word_id_map.get(word_j) is not None:
...
def get_random_r(args):
    num_nodes, num_rels = get_total_number(args.dp + args.dn, args.ct + 'stat.txt')
...
def get_tr_dataset(args, set_name):
    num_nodes, num_rels = get_total_number(args.dp + args.dn, args.ct + 'stat.txt')
    file_path = '{}{}/tr_data_{}{}_sl{}.npy'.format(args.dp, args.dn, args.ct, set_name, args.seq_len)
    print(file_path)
    if not os.path.exists(file_path):
        print('build tr_data ...', args.dn, args.ct, set_name)
        data, times = load_quadruples(args.dp + args.dn, args.ct + set_name + '.txt')  # triplets
        filename = '{}{}/{}{}_history_rel_{}.txt'.format(args.dp, args.dn, args.ct, set_name, args.seq_len)
        with open('{}{}/{}{}_history_rel_{}.txt'.format(args.dp, args.dn, args.ct, set_name, args.seq_len),
                  'rb') as f:  # r history data
        ...
        print('load tr_data ...', args.dn, args.ct, set_name)
...
def get_scaled_tr(args, r_list, set_name='train'):
    new_file_path = args.dp + args.dn + '/tr_data_{}_sl{}_rand_{}.npy'.format(args.ct + set_name, args.seq_len, args.k)
    if not check_exist(new_file_path):
        # load tr_datasets
        file_path = args.dp + args.dn + '/tr_data_{}_sl{}.npy'.format(args.ct + set_name, args.seq_len)
        with open(file_path, 'rb') as f:
            [t_data, r_data, r_hist, r_hist_t, true_prob_s, true_prob_o] = pickle.load(f)
        new_t_data = []
        new_r_data = []
...
def build_scaled_wg_r(args, r_list):
    ...
    new_file_path = args.dp + args.dn + '/' + args.ct + 'wg_r_dict_sl{}_rand_{}.txt'.format(args.seq_len, args.k)
    if not check_exist(new_file_path):
        tokens_l, vocab_l = get_token_vocab(args)
        with open(args.dp + args.dn + '/' + args.ct + 'dg_dict.txt', 'rb') as f:
...
def get_scaled_g_r(args, r_list):
    ...
    new_file_path = args.dp + args.dn + '/' + args.ct + 'dg_r_dict_sl{}_rand_{}.txt'.format(args.seq_len, args.k)
    if not check_exist(new_file_path):
        file_path = args.dp + args.dn + '/' + args.ct + 'dg_r_dict.txt'
        with open(file_path, 'rb') as f:
            graph_dict = pickle.load(f)
        new_graph_dict = {}
        for r in r_list:
...
def saveRlist(args, r_list):
    if not r_list:
        ...
        exit()
    file_path = args.dp + args.dn + '/' + args.ct + 'r_list_sl{}_rand_{}.txt'.format(args.seq_len, args.k)
...
if __name__ == "__main__":
    ...
    # save it ......
    # build data for baseline methods
    saveRlist(args, r_list)
