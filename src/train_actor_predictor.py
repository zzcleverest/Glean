...
parser.add_argument("--dn", type=str, default="dataset", help="dataset name")
parser.add_argument("--ct", type=str, default="IR_", help="country prefix")
...
parser.add_argument("-d", "--dataset", type=str, default='dataset', help="dataset to use")
...
while iterations < args.runs:
    ...
    if iterations == 1:
        ...
        num_nodes, num_rels = utils.get_total_number(
            args.dp + args.dn, args.ct + 'stat.txt')

        with open('{}{}/{}100.w_emb'.format(args.dp, args.dn, args.ct), 'rb') as f:
            word_embeds = pickle.load(f, encoding="latin1")
        word_embeds = torch.FloatTensor(word_embeds)
        vocab_size = word_embeds.size(0)

        train_dataset_loader = EntDistGivenTRData(
            args.dp, args.dn, num_nodes, num_rels, args.ct + 'train', args.seq_len, num_r=args.num_r)
        valid_dataset_loader = EntDistGivenTRData(
            args.dp, args.dn, num_nodes, num_rels, args.ct + 'valid', args.seq_len, num_r=args.num_r)
        test_dataset_loader = EntDistGivenTRData(
            args.dp, args.dn, num_nodes, num_rels, args.ct + 'test', args.seq_len, num_r=args.num_r)
        ...
        with open(args.dp + args.dn + '/{}wg_r_dict_sl{}_rand_{}.txt'.format(args.ct, args.seq_len, args.num_r),
                  'rb') as f:
            word_graph_dict = pickle.load(f)
        with open(args.dp + args.dn + '/{}dg_r_dict_sl{}_rand_{}.txt'.format(args.ct, args.seq_len, args.num_r),
                  'rb') as f:
            graph_dict = pickle.load(f)

        # load word_relation_map.txt
        with open(args.dp + args.dn + '/' + args.ct + 'word_relation_map.txt', 'rb') as f:
            rel_map = pickle.load(f)
        # load word_entity_map.txt
        with open(args.dp + args.dn + '/' + args.ct + 'word_entity_map.txt', 'rb') as f:
    ...
    token = '{}{}_sl{}_max{}_gru{}_attn{}'.format(args.ct, model_name, args.seq_len, int(args.maxpool),
                                                  int(args.use_gru), str(args.attn))

    print('Token:', token, args.dn)
    ...
    os.makedirs('models/' + args.dn, exist_ok=True)
    model_state_file = 'models/{}/{}.pth'.format(args.dn, token)
    model_graph_file = 'models/{}/{}_graph.pth'.format(args.dn, token)
    outf = 'models/{}/{}.result'.format(args.dn, token)
    if use_cuda:
        model.cuda()
    ...
    def evaluate(data_loader, dataset_loader, set_name='valid'):
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            true_rank_s, prob_rank_s, true_rank_o, prob_rank_o, ranks, loss = model.evaluate(t_data, r_data, r_hist,
                                                                                             r_hist_t, true_s, true_o)
