...
def get_stopwords_basic():
    file = '../data/stopwords-en-basic.txt'
...
def get_s2v_model(args):
    ...
    model_file = os.path.join(args.dp + args.dn, args.ct + 's2v_{}.bin'.format(args.dim))
...
def get_sent_voc_vectors(args):
    ...
    text_f = os.path.join(args.dp + args.dn, args.ct + 'text_token.txt')
    ...
    text_vocab_file = os.path.join(args.dp + args.dn, args.ct + 'vocab.txt')
    ...
    word_vec_file = os.path.join(args.dp + args.dn, args.ct + '{}.w_emb'.format(args.dim))
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(args.dp, args.dn, args.ct + 'text.txt')):
